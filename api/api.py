...
from api.constants import Identifier, Field, Url, Endpoint
...
async def get_tariff_data_today() -> list[TariffData]:
    ...
    df = await get_data(f"{Url.REST_API}{Endpoint.STANDARD_UNIT_RATES}",
                        start_datetime, end_datetime, Field.VALID_FROM, Field.VALUE)
    return df.to_dicts()
...
@async_cache
async def get_smart_meter_usage_historic(start_datetime: datetime, end_datetime: datetime) -> list[ConsumptionData]:
