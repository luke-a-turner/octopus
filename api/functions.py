...
from api.constants import Url, Endpoint, Field, Identifier, PAGE_SIZE, SECONDS_IN_HOUR
...
async def get_data(url: str, start_datetime: datetime, end_datetime: datetime, date_field: Field, value_field: Field) -> pl.DataFrame:
    """ Get Octopus Energy data """
    # number of half hour report intervals between start and end datetime
    number_data_points = ceil(((end_datetime - start_datetime).total_seconds() / SECONDS_IN_HOUR) * 2)
    ...
    df = (
        pl.concat(lazy_frames)
            ...
            .filter((pl.col(date_field) > start_datetime) &
                    (pl.col(date_field) < end_datetime))
